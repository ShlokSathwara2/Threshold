...
const RippleDistortion = ({
  ...
  useEffect(() => {
    ...
    import('ogl').then(({ Renderer, Program, Mesh, Geometry, Triangle, Texture, RenderTarget }) => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const compositeUniforms: Record<string, any> = {
      ...
      const resize = () => {
        ...
        (compositeUniforms.uResolution.value as number[]) = [width, height];
        ...
        (compositeUniforms.uTexel.value as number[]) = [1 / fieldW, 1 / fieldH];
      };
      ...
      const setNewWave = (x: number, y: number, power: number) => {
        ...
        wave.target = START_SCALE * Math.max(1, cfg.spread as number) * power;
        wave.size = Math.max(1, cfg.brushSize as number);
        wave.opacity = 1;
      };
      ...
      const loop = (now: number) => {
        ...
        const decay = reduceMotion ? 1 : Math.exp((-delta * LIFE_CONSTANT) / Math.max(0.15, cfg.fade as number));
      ...
      // Store cleanup
      (mount as unknown as Record<string, unknown>).__rippleCleanup = () => {
        disposed = true;
    ...
    return () => {
      const cleanup = (mount as unknown as Record<string, () => void>).__rippleCleanup;
      if (cleanup) cleanup();
    };
  ...
  useEffect(() => {
    ...
    if (!u) return;
    const w = u.wave as Record<string, { value: number }>;
    const c = u.composite as Record<string, { value: unknown }>;
